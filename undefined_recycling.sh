...
cat ./undefined_recycling/recycling_type_no_amenity.json | grep -e '"recycling:' -e 'FeatureCollection' -e '^]}$' | grep -vi "recycling_type"  | grep -vi '"amenity":"recycling"' | tac | sed '2s/,$//' | tac | jq . > ./undefined_recycling/recycling_type_no_amenity.geojson
